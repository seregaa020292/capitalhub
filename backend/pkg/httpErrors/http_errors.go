...
const (
	...
	ErrEmailNotConfirmed  = "User with given email not confirmed"
...
var (
	...
	ExpiredJWTToken       = errors.New("JWT Token is expired")
	FingerprintNotMatch   = errors.New("Fingerprint does not match")
	...
	DuplicateUniqueError  = errors.New("Duplicate key value violates unique")
	InvalidJWTToken       = errors.New("Invalid JWT token")
	InvalidJWTClaims      = errors.New("Invalid JWT claims")
	EmptyJWTToken         = errors.New("Empty JWT token")
...
func parseSqlErrors(err error) RestErr {
	if strings.Contains(err.Error(), "23505") {
		return NewRestError(http.StatusBadRequest, DuplicateUniqueError.Error(), err)
	}
