...
import (
	...
	"github.com/seregaa020292/capitalhub/pkg/sanitize"
)
...
func GetRequestCtx(ctx echo.Context) context.Context {
	return context.WithValue(ctx.Request().Context(), ReqIDCtxKey{}, GetRequestID(ctx))
}

// Configure refresh token cookie
func SetRefreshTokenCookie(cfg *config.Config, token string) *http.Cookie {
	return &http.Cookie{
		Name:       cfg.Auth.NameRefreshToken,
		Value:      token,
		Path:       "/",
		RawExpires: "",
...
// Delete session
func DeleteSessionCookie(ctx echo.Context, sessionName string) {
...
func SetCSRFHeader(echoCtx echo.Context, sid string, salt string, logger logger.Logger) {
	token := csrf.MakeToken(sid, salt, logger)
